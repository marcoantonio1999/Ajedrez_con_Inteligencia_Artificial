...
class Box():
    ...
    def setImage(self):
        ...
        self.img_box = PhotoImage(file=relativeToAssets("images/"+piece+".png"))

    """
    Muestra en pantalla la casilla
    """
    def showOnScreen(self):
        self.canvas_box =  Button(image=self.img_box,borderwidth=0,highlightthickness=0,command= lambda id = (8*self.x)+self.y: self.onClick(id),relief="flat")
