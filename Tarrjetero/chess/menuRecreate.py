...
class MenuRecreate:
    ...
    def __init__(self, canvas, board) -> None:
        ...
        self.selectPiece =PhotoImage(file="assets/images/selection.png")

        #Img para borrar una pieza

        self.deletePiece  =PhotoImage(file="assets/images/eraser.png")

        #Img para cerrar el menú

        self.close  =PhotoImage(file="assets/images/close.png")
    ...
    def setPieces(self, y, color):
        ...
        for i in range(6):
            ...
            self.imgButtons.append(PhotoImage(file="assets/images/" + name + "0.png"))
            self.buttons.append(Button(self.canvas, image=self.imgButtons[p+i],borderwidth=0,highlightthickness=0,command= lambda x = name: self.doClick(x),
                relief="flat"
            ))
    ...
    def setMenu(self):
        ...
        for i in range(2):
            aux = not aux
            self.buttons.append(Button(self.canvas, image=self.selectPiece if i == 0 else self.deletePiece,
                borderwidth=0,
        ...
        self.buttons.append(Button(self.canvas, image=self.close,borderwidth=0,highlightthickness=0,command= self.deleteMenu,relief="flat"))
